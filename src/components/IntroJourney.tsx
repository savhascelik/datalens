...
// Anasayfa (Dosyalar sekmesi boşken) gösterilen animasyonlu "yolculuk" hero'su:
// Veri → (AI) → Pano → Rapor. Saf CSS animasyonları (styles.css içindeki .dlj-* / @keyframes),
// ek bağımlılık yok. Jüriye ürünün akışını tek bakışta anlatır.

import { Database, LayoutGrid, FileText, Sparkles } from 'lucide-react'

const Node = ({ icon, title, sub, delay, accent }: { icon: React.ReactNode; title: string; sub: string; delay: number; accent?: boolean }) => (
  <div className="dlj-node" style={{ animationDelay: `${delay}ms`, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', minWidth: '92px', textAlign: 'center' }}>
    <div className="dlj-icon" style={{
      width: '58px', height: '58px', borderRadius: '16px', display: 'grid', placeItems: 'center',
    ...
    <div>
      ...
      <div style={{ fontSize: '11px', color: 'var(--text-muted)', maxWidth: '120px' }}>{sub}</div>
    </div>
  </div>
)
...
// Minik "pano" ikonu: büyüyen barlar (animasyonlu).
const MiniDashboard = () => (
  <div style={{ display: 'flex', alignItems: 'flex-end', gap: '4px', height: '26px' }}>
    {[0.5, 0.9, 0.65, 1].map((h, i) => (
      <span key={i} className="dlj-bar" style={{ animationDelay: `${700 + i * 120}ms`, width: '5px', height: `${h * 26}px`, borderRadius: '2px', background: 'var(--color-primary-dark)' }} />
    ))}
  </div>
)

export function IntroJourney({ t }: { t: (key: string, options?: any) => string }) {
  return (
    <div className="dlj-hero card" style={{
      ...
      <div className="dlj-flow" style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
        <Node delay={80} icon={<Database size={26} />} title={t('intro.data', { defaultValue: 'Veri' })} sub={t('intro.dataSub', { defaultValue: 'CSV / Excel / Parquet' })} />
        <Connector delay={0} />
        ...
        <Node delay={440} icon={<FileText size={26} />} title={t('intro.report', { defaultValue: 'Rapor' })} sub={t('intro.reportSub', { defaultValue: 'Anlatı + kart, temiz PDF' })} />
      </div>
    </div>
  )
}
